...
2026-05-05T11:16:34+00:00 — 公開（gh-pages ブランチへデプロイ）
- やったこと: production ビルドを作成し、dist の内容を gh-pages ブランチにプッシュしました。ビルドには public/assets のプレースホルダ音声が含まれます。
- 備考: GitHub Pages の API 設定は環境の権限不足により自動設定できませんでした（HTTP 403）。リポジトリの Settings > Pages でブランチが gh-pages に設定されているか確認してください。公開 URL(通常): https://ramen0129.github.io/ramen20260505/
- 次のステップ: ライブ確認とプレースホルダ音声の差し替え、必要なら Pages 設定の有効化。

2026-05-05T11:24:22+00:00 — プレースホルダ音声差し替えと再デプロイ
- やったこと: public/assets に WAV で BGM/SFX を生成して追加（bgm-loop.wav, sfx-hit.wav）。src/game.ts を .wav に更新、ビルドして gh-pages にプッシュ。公開 URL にアクセスし HTTP/2 200 を確認。
- 次のステップ: 実音源差し替え、ユーザーテスト、README 追記。
